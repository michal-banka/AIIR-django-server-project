...
urlpatterns = [
    ...
    url(r'^(?P<input>[\w]+)/input/(?P<taskId>[0-9]+)$', csrf_exempt(views.input_data), name='input_data'),
    url(r'^(?P<input>[\w]+)/output/(?P<taskId>[0-9]+)$', csrf_exempt(views.output_data), name='output_data'),
